...
import CustomQuestionCheckBox from '../InspectionComponents/CustomQuestionCheckBox';
import { useLoader } from '../../../lib/loaderContext';
import axios from 'axios';
import { apiUrl } from '../../../constants/api_Url';
const AddNewQuestion = ({ route }) => {
    const { setLoading } = useLoader()
    const currentRole = 'FREETIER'
    //  const currentRole ='STANDARDTIER'
    //   const currentRole ='TOPTIER'
    //   const currentRole ='SUBUSER'
    const { inspectionID, roomID, elementId, questionArray, saveQuestionArray ,elementIndex} = route?.params
    const navigation = useNavigation();
    const [selectedValue, setSelectedValue] = useState(null);
    const [selectedCheckBoxValue, setSelectedCheckBoxValue] = useState(null);
    const [isFormValid, setIsFormValid] = useState(false);
    // const questionArr =questionArray[0]?.checklist
    // console.log('questionArray',questionArray[0]?.checklist);
    // console.log('saveQuestionArray',saveQuestionArray);
    // console.log('---------questionArray',questionArray[0]?.checklist);
    // console.log('---------saveQuestionArray',saveQuestionArray);
    ...
    // console.log('saveQuestionArr',saveQuestionArr);
    ...
    const handleSave = async () => {
        ...
        const params = {
            inspectionId: inspectionID, // Replace with actual values
            roomId: roomID,
            elementId: elementId,
            questions: [
                {
        ...
        try {
            const response = await axios.post(`${apiUrl}/api/inspection/InspectionAddNewChecklistItem`, params, { withCredentials: true });

            if (response?.status === 200 || response?.status === 201) {
                // navigation.navigate('CreateRoomInspection')
                navigation.goBack()
            }
        } catch (error) {
    ...
    const handleSelectedCheckBox = useCallback((value, item) => {
        // console.log('value------------>>>>',item?.options[0]?._id);
        ...
        <SafeAreaView style={styles.mainContainer}>
            <ScrollView contentContainerStyle={{ flexGrow: 1 }} showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.inputContainer}>
                  {saveQuestionArray?.length ===0 ?  <Text style={styles.inputLabelText}>
                    ...
                    {currentRole && currentRole !== 'SUBUSER' ?
                        <TouchableOpacity
                            style={[styles.transparentButton]}
                            onPress={() => {
                                navigation.navigate("CreateNewQuestion", {
                                    inspectionID,
                                    roomID,
                                    elementId
                                });
