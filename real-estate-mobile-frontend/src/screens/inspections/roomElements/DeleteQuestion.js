import { SafeAreaView, StyleSheet, Text, TouchableOpacity, View, ScrollView, ToastAndroid } from 'react-native';
...
const DeleteQuestion = () => {
    ...
    const { inspectionID, roomID, elementId, questionArray ,elementIndex} = route?.params

    const { setLoading } = useLoader()
    const [questionsArr, setQuestionsArr] = useState(questionArray?.[elementIndex]?.checklist);
        ...
        if ((inspectionID && roomID)) {
            ...
            try {
                ...
                if (response?.status === 200 || response?.status === 201) {
                    setQuestionsArr(response?.data?.room?.elements?.[elementIndex]?.checklist)
                    //  setLoading(false);
    ...
    const deleteInspectionQuestion = async () => {
        if ((selectedItems)) {
            ...
            const data = {
                ...
                roomId: roomID ? roomID : null,
                elementId: elementId,
                checklistIdArray: selectedItems
            };

            try {
                const response = await axios.post(`${apiUrl}/api/inspection/InspectionDeleteChecklistItem`, data, { withCredentials: true });
                if (response?.status === 200 || response?.status === 201) {
                    setSelectedItems([])
                    getInspectionRoomData()
    ...
    //         try {
    //             const response = await axios.delete(`${apiUrl}/api/inspection/deleteSavedQuestion/${selectedItems}`,);
    //             if (response?.status === 200 || response?.status === 201) {
        ...
        <SafeAreaView style={styles.mainContainer}>
            <ScrollView contentContainerStyle={{ flexGrow: 1 }} showsVerticalScrollIndicator={false}>
                <View style={styles.innerContainer}>
                    <Text style={styles.inputTextLabel}>Questions</Text>

                    {questionsArr?.length > 0 && questionsArr?.map((item, index) => {
                            ...
                            >
                                ...
                                <View style={styles.searchContainer}>
                                    <View style={styles.searchInput}>
                                        <Text style={[styles.searchInput, { padding: 0 }]}>{index + 1}.</Text>
                ...
                <TouchableOpacity
                    style={[styles.newInspectionButton, selectedItems?.length > 0 && { backgroundColor: '#FF613E' }]}
                    disabled={selectedItems?.length === 0}
                    onPress={deleteInspectionQuestion}
                >
                    <Text style={styles.newInspectionButtonText}>Delete Selected Questions</Text>
                </TouchableOpacity>
