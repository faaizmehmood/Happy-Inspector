    ...
    const handleDelete = async () => {
        if (route?.params?.templateID) {
            try {
                ...
                const response = await axios.patch(`${apiUrl}/api/template/templateDeleteRoom`, data, { withCredentials: true })
                if (response?.status === 200) {
                    setSelectedItems([])
                    showingRooms();
                }
...
export default DeleteTemplateRoom;
