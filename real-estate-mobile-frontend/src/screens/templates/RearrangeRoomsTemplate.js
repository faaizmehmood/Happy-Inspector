...
const RearrangeRoomsTemplate = () => {
  ...
  useEffect(() => {
    const isSameOrder = data.every(
      (item, index) => item?._id === roomsArray[index]?._id
    );
    ...
    if (route?.params?.templateID) {
      ...
      try {
        const response = await axios.get(
          `${apiUrl}/api/template/getSpecificTemplate/${id}`,
          { withCredentials: true }
        );
        if (response?.status === 200) {
          setRoomsArray(response?.data?.rooms);
          if (response?.data?.rooms?.length === 0) {
            navigation.navigate("ShowRooms", { id: route?.params?.templateID });
            setLoading(false);
          }
        }
      } catch (error) {
    ...
    if (route?.params?.templateID) {
      try {
        ...
        const response = await axios.patch(
          `${apiUrl}/api/template/reArrangeRooms`,
          data,
          { withCredentials: true }
        );
        if (response?.status === 200) {
    ...
    <View style={styles.checkBoxParentContainer}>
      ...
      <View
        style={[
          ...
          {
            backgroundColor:
              data?.length > 1 && isActive ? "#DAEAFF" : "#FFFFFF",
          },
        ]}
      >
...
export default RearrangeRoomsTemplate;
